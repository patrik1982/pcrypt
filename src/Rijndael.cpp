...
#define XOR128BIT(DEST,OP1,OP2) (*(__m128i*)&DEST = _mm_xor_si128(*(__m128i*)OP1, *(__m128i*)OP2))

using namespace std;

Rijndael::Rijndael() : IAESCandidate()
{
	number_of_rounds_ = 0;
}
...
void Rijndael::EncryptBlock(vector<uint8_t> &ciphertext, const vector<uint8_t> &plaintext)
{
	...
	if (use_sse4_aes_ni_) {
		...
		pt = (__m128i *)pbuf;
		tmp = _mm_xor_si128(*pt, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		...
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		if (variant_ == Key192) {		// Two more rounds
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		}
		else if (variant_ == Key256) {	// Four more rounds
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
			tmp = _mm_aesenc_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		}
		tmp = _mm_aesenclast_si128(tmp, *(__m128i *)&encryption_keys_[round++ * 4]);
		((__m128i *)cbuf)[0] = tmp;

	} else {

		uint32_t dwbuf[4];
		uint8_t *ibbuf = (uint8_t *)dwbuf;
		uint8_t *bbuf = ciphertext.data();

		XOR128BIT(dwbuf, pbuf, &encryption_keys_[round * 4]);
		for (round = 1; round < number_of_rounds_ - 1; round++) {
			cbuf[0] = TBox0[ibbuf[0]] ^ TBox1[ibbuf[5]] ^ TBox2[ibbuf[10]] ^ TBox3[ibbuf[15]];
			cbuf[1] = TBox0[ibbuf[4]] ^ TBox1[ibbuf[9]] ^ TBox2[ibbuf[14]] ^ TBox3[ibbuf[3]];
			cbuf[2] = TBox0[ibbuf[8]] ^ TBox1[ibbuf[13]] ^ TBox2[ibbuf[2]] ^ TBox3[ibbuf[7]];
			cbuf[3] = TBox0[ibbuf[12]] ^ TBox1[ibbuf[1]] ^ TBox2[ibbuf[6]] ^ TBox3[ibbuf[11]];
			dwbuf[0] = cbuf[0] ^ encryption_keys_[round * 4 + 0];
			dwbuf[1] = cbuf[1] ^ encryption_keys_[round * 4 + 1];
			dwbuf[2] = cbuf[2] ^ encryption_keys_[round * 4 + 2];
			dwbuf[3] = cbuf[3] ^ encryption_keys_[round * 4 + 3];
		}

		bbuf[0] = Sbox[ibbuf[0]];
		bbuf[1] = Sbox[ibbuf[5]];
		bbuf[2] = Sbox[ibbuf[10]];
		bbuf[3] = Sbox[ibbuf[15]];
		bbuf[4] = Sbox[ibbuf[4]];
		bbuf[5] = Sbox[ibbuf[9]];
		bbuf[6] = Sbox[ibbuf[14]];
		bbuf[7] = Sbox[ibbuf[3]];
		bbuf[8] = Sbox[ibbuf[8]];
		bbuf[9] = Sbox[ibbuf[13]];
		bbuf[10] = Sbox[ibbuf[2]];
		bbuf[11] = Sbox[ibbuf[7]];
		bbuf[12] = Sbox[ibbuf[12]];
		bbuf[13] = Sbox[ibbuf[1]];
		bbuf[14] = Sbox[ibbuf[6]];
		bbuf[15] = Sbox[ibbuf[11]];

		cbuf[0] ^= encryption_keys_[round * 4 + 0];
		cbuf[1] ^= encryption_keys_[round * 4 + 1];
		cbuf[2] ^= encryption_keys_[round * 4 + 2];
		cbuf[3] ^= encryption_keys_[round * 4 + 3];
	}
}

void Rijndael::DecryptBlock(vector<uint8_t> &plaintext, const vector<uint8_t> &ciphertext)
{
	uint32_t *cbuf = (uint32_t*)ciphertext.data();
	uint32_t *pbuf = (uint32_t*)plaintext.data();
	size_t round = number_of_rounds_ - 1;
	...
	if (use_sse4_aes_ni_) {
		...
		ct = (__m128i *)cbuf;
		tmp = _mm_xor_si128(*ct, *(__m128i *)&decryption_keys_[round-- * 4]);
		tmp = _mm_aesdec_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		tmp = _mm_aesdec_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		tmp = _mm_aesdec_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		tmp = _mm_aesdec_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		tmp = _mm_aesdec_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		...
		tmp = _mm_aesdeclast_si128(tmp, *(__m128i *)&decryption_keys_[round-- * 4]);
		((__m128i *)pbuf)[0] = tmp;
		...
		uint32_t dwbuf[4];
		uint8_t *ibbuf = (uint8_t *)dwbuf;
		uint8_t *bbuf = plaintext.data();

		XOR128BIT(dwbuf, cbuf, &decryption_keys_[round * 4]);
		for (round -= 1; round > 0; round--) {
			pbuf[0] = invTBox0[ibbuf[0]] ^ invTBox1[ibbuf[13]] ^ invTBox2[ibbuf[10]] ^ invTBox3[ibbuf[7]];
			pbuf[1] = invTBox0[ibbuf[4]] ^ invTBox1[ibbuf[1]] ^ invTBox2[ibbuf[14]] ^ invTBox3[ibbuf[11]];
			pbuf[2] = invTBox0[ibbuf[8]] ^ invTBox1[ibbuf[5]] ^ invTBox2[ibbuf[2]] ^ invTBox3[ibbuf[15]];
			pbuf[3] = invTBox0[ibbuf[12]] ^ invTBox1[ibbuf[9]] ^ invTBox2[ibbuf[6]] ^ invTBox3[ibbuf[3]];
			dwbuf[0] = pbuf[0] ^ decryption_keys_[round * 4 + 0];
			dwbuf[1] = pbuf[1] ^ decryption_keys_[round * 4 + 1];
			dwbuf[2] = pbuf[2] ^ decryption_keys_[round * 4 + 2];
			dwbuf[3] = pbuf[3] ^ decryption_keys_[round * 4 + 3];
		}

		bbuf[0] = invSbox[ibbuf[0]];
		bbuf[1] = invSbox[ibbuf[13]];
		bbuf[2] = invSbox[ibbuf[10]];
		bbuf[3] = invSbox[ibbuf[7]];
		bbuf[4] = invSbox[ibbuf[4]];
		bbuf[5] = invSbox[ibbuf[1]];
		bbuf[6] = invSbox[ibbuf[14]];
		bbuf[7] = invSbox[ibbuf[11]];
		bbuf[8] = invSbox[ibbuf[8]];
		bbuf[9] = invSbox[ibbuf[5]];
		bbuf[10] = invSbox[ibbuf[2]];
		bbuf[11] = invSbox[ibbuf[15]];
		bbuf[12] = invSbox[ibbuf[12]];
		bbuf[13] = invSbox[ibbuf[9]];
		bbuf[14] = invSbox[ibbuf[6]];
		bbuf[15] = invSbox[ibbuf[3]];

		pbuf[0] ^= decryption_keys_[round * 4 + 0];
		pbuf[1] ^= decryption_keys_[round * 4 + 1];
		pbuf[2] ^= decryption_keys_[round * 4 + 2];
		pbuf[3] ^= decryption_keys_[round * 4 + 3];

	}
}

void Rijndael::SetKey(const vector<uint8_t> &key)
{
	vector<uint8_t> prekey = key;
	switch (prekey.size()) {
		case 16:
			number_of_rounds_ = 11;
			variant_ = Key128;
			break;
		case 24:
			number_of_rounds_ = 13;
			variant_ = Key192;
			break;
		default:
			if (prekey.size() < 32) {
				prekey.push_back(0x01);
				while (prekey.size() < 32) {
					prekey.push_back(0x00);
				}
			}
			number_of_rounds_ = 15;
			variant_ = Key256;
			break;
	}

	encryption_keys_ = vector<uint32_t>(number_of_rounds_ * 4);
	decryption_keys_ = vector<uint32_t>(number_of_rounds_ * 4);

	CalculateRoundKeys((uint32_t*)prekey.data());
}

void Rijndael::Clear()
{
	for (size_t i = 0; i < encryption_keys_.size(); i++) {
		encryption_keys_[i] = 0;
	}
	for (size_t i = 0; i < decryption_keys_.size(); i++) {
		decryption_keys_[i] = 0;
	}
}

void Rijndael::CalculateRoundKeys(const uint32_t * key)
{
	uint32_t* wbuf;
	uint32_t* wbufp;
	...
	switch (variant_) {
	case Key128:
		encryption_keys_[0] = key[0];
		encryption_keys_[1] = key[1];
		encryption_keys_[2] = key[2];
		encryption_keys_[3] = key[3];

		for (int i = 1; i<11; i++) {
			wbufp = &encryption_keys_[4 * (i - 1)];
			wbuf = &encryption_keys_[4 * i];

			wbuf[0] = wbufp[0] ^ g(wbufp[3], RC[i]);
			wbuf[1] = wbuf[0] ^ wbufp[1];
			wbuf[2] = wbuf[1] ^ wbufp[2];
			wbuf[3] = wbuf[2] ^ wbufp[3];
		}
		break;
	...
	case Key192:
		...
		encryption_keys_[2] = key[2];
		encryption_keys_[3] = key[3];
		encryption_keys_[4] = key[4];
		encryption_keys_[5] = key[5];

		for (int i = 1; i < 8; i++) {
			wbufp = &encryption_keys_[6 * (i - 1)];
			wbuf = &encryption_keys_[6 * i];

			wbuf[0] = wbufp[0] ^ g(wbufp[5], RC[i]);
			wbuf[1] = wbuf[0] ^ wbufp[1];
			wbuf[2] = wbuf[1] ^ wbufp[2];
			wbuf[3] = wbuf[2] ^ wbufp[3];
			wbuf[4] = wbuf[3] ^ wbufp[4];
			wbuf[5] = wbuf[4] ^ wbufp[5];
		}
		wbufp = &encryption_keys_[6 * (8 - 1)];
		wbuf = &encryption_keys_[6 * 8];
		wbuf[0] = wbufp[0] ^ g(wbufp[5], RC[8]);
		wbuf[1] = wbuf[0] ^ wbufp[1];
		wbuf[2] = wbuf[1] ^ wbufp[2];
		wbuf[3] = wbuf[2] ^ wbufp[3];
		break;
	...
	case Key256:
		...
		encryption_keys_[4] = key[4];
		encryption_keys_[5] = key[5];
		encryption_keys_[6] = key[6];
		encryption_keys_[7] = key[7];

		for (int i = 1; i < 7; i++) {
			wbufp = &encryption_keys_[8 * (i - 1)];
			wbuf = &encryption_keys_[8 * i];

			wbuf[0] = wbufp[0] ^ g(wbufp[7], RC[i]);
			wbuf[1] = wbuf[0] ^ wbufp[1];
			wbuf[2] = wbuf[1] ^ wbufp[2];
			wbuf[3] = wbuf[2] ^ wbufp[3];
			wbuf[4] = h(wbuf[3]) ^ wbufp[4];
			wbuf[5] = wbuf[4] ^ wbufp[5];
			wbuf[6] = wbuf[5] ^ wbufp[6];
			wbuf[7] = wbuf[6] ^ wbufp[7];
		}
		wbufp = &encryption_keys_[8 * (7 - 1)];
		wbuf = &encryption_keys_[8 * 7];
		wbuf[0] = wbufp[0] ^ g(wbufp[7], RC[7]);
		wbuf[1] = wbuf[0] ^ wbufp[1];
		wbuf[2] = wbuf[1] ^ wbufp[2];
		wbuf[3] = wbuf[2] ^ wbufp[3];
		break;
	...
	size_t i;
	decryption_keys_[0] = encryption_keys_[0];
	decryption_keys_[1] = encryption_keys_[1];
	decryption_keys_[2] = encryption_keys_[2];
	decryption_keys_[3] = encryption_keys_[3];
	for (i = 1; i < number_of_rounds_ - 1; i++) {
		decryption_keys_[i * 4 + 0] = encryption_keys_[i * 4 + 0];
		decryption_keys_[i * 4 + 1] = encryption_keys_[i * 4 + 1];
		decryption_keys_[i * 4 + 2] = encryption_keys_[i * 4 + 2];
		decryption_keys_[i * 4 + 3] = encryption_keys_[i * 4 + 3];
		InvMixColumns(&decryption_keys_[i * 4]);
	}
	decryption_keys_[i * 4 + 0] = encryption_keys_[i * 4 + 0];
	decryption_keys_[i * 4 + 1] = encryption_keys_[i * 4 + 1];
	decryption_keys_[i * 4 + 2] = encryption_keys_[i * 4 + 2];
...
uint32_t Rijndael::g(const uint32_t w, const uint8_t rc)
{
	uint32_t v;
	uint8_t* b;
	...
	v = w;
	b = (uint8_t*)&v;
	v = (Sbox[b[1]] ^ rc) | (Sbox[b[2]] << 8) | (Sbox[b[3]] << 16) | (Sbox[b[0]] << 24);
	return v;
}

uint32_t Rijndael::h(const uint32_t w)
{
	uint32_t v;
	uint8_t* b;
...
void Rijndael::InvMixColumns(uint32_t * data)
{
	...
	for (int i = 0; i < 4; i++) {
		...
		b2 = bbuf[i * 4 + 2];
		b3 = bbuf[i * 4 + 3];
		bbuf[i * 4 + 0] = GFmult0x0e[b0] ^ GFmult0x0b[b1] ^ GFmult0x0d[b2] ^ GFmult0x09[b3];
		bbuf[i * 4 + 1] = GFmult0x09[b0] ^ GFmult0x0e[b1] ^ GFmult0x0b[b2] ^ GFmult0x0d[b3];
		bbuf[i * 4 + 2] = GFmult0x0d[b0] ^ GFmult0x09[b1] ^ GFmult0x0e[b2] ^ GFmult0x0b[b3];
		bbuf[i * 4 + 3] = GFmult0x0b[b0] ^ GFmult0x0d[b1] ^ GFmult0x09[b2] ^ GFmult0x0e[b3];
	}
}

const uint8_t Rijndael::RC[] = {
	0x8d, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36, 0x6c, 0xd8, 0xab, 0x4d, 0x9a
};

const uint8_t Rijndael::Sbox[] = {
...
const uint8_t Rijndael::invSbox[256] = {
...
const uint32_t Rijndael::invTBox0[] = {
...
const uint32_t Rijndael::invTBox1[] = {
...
const uint32_t Rijndael::invTBox2[] = {
...
const uint32_t Rijndael::invTBox3[] = {
...
const uint8_t Rijndael::GFmult0x09[] = {
...
const uint8_t Rijndael::GFmult0x0b[] = {
...
const uint8_t Rijndael::GFmult0x0d[] = {
...
const uint8_t Rijndael::GFmult0x0e[] = {
